...
CREATE TABLE mv_memb (
    ...
    age_rng VARCHAR(60)  NULL     COMMENT '연령대' -- 연령대
)
COMMENT '회원';
...
CREATE TABLE mv_mv_anly (
    ...
    pnt  INTEGER NULL     COMMENT '점수' -- 점수
)
COMMENT '영화분석';
...
-- 게시물_친구태그
ALTER TABLE mv_post_flw_tag
    ADD CONSTRAINT FK_mv_post_TO_mv_post_flw_tag -- 게시물 -> 게시물_친구태그
        FOREIGN KEY (
            pstno -- 게시물번호
        )
        REFERENCES mv_post ( -- 게시물
...
-- 게시물_친구태그
ALTER TABLE mv_post_flw_tag
    ADD CONSTRAINT FK_mv_memb_TO_mv_post_flw_tag -- 회원 -> 게시물_친구태그
        FOREIGN KEY (
            flwno -- 팔로우번호
        )
        REFERENCES mv_memb ( -- 회원
