...
function makeMovieListHtml(data) {
  ...
  data.movieList.forEach(function(obj, idx) {
    ...
    html += ' name="mvList" class="badge badge-primary badge-pill" style="cursor: 
...
function makeFavListHtml(id, title) {
  ...
  print += `<button type="button" onclick="removeList('` + id +`')" style="float:right; cursor: 
...
/*=======
  ...
  function makeFavListHtml(id, title) {
    ...
    print += '<div class="media-body">';
    print += '<span class="mt-0"><b>' + title + "\t" + '</b></span>';
    print += `<button type="button" onclick="removeList('` + id +`')" style="float:right; 
