...
<div id="pstShw">
			...
			<div class="row">
				<div class="col text-center pst"
					...
					<i class="far fa-image"></i> 일상 리뷰 올리기 <input type="hidden"
						data-toggle="modal" id="btnIlsang" onclick="postShow('btnIlsang')"
	...
	<c:forEach items="${postList}" var="post" varStatus="status">
		...
		<div class="wPost reviewPst aos-init" data-aos="fade-up"
			...
			<div class="media row pr-3 pl-3">
					...
					<ul class="memberul">
						<li><span onclick="goToFeed(${post.member.mno})"
							...
							class="cmt-date">&nbsp; <fmt:formatDate
									pattern="yyyy-MM-dd hh:mm:ss" value="${post.createdDate}" />
						</span> <%-- =================================== POST HEADER ========================================== --%>

							<%-- ==================================== POST OPEN? ========================================== --%>
						</li>
						<li><c:if test="${post.open == false}">
								<i id="lock-${post.pstno}" class="fas fa-lock lock d-inline">&nbsp;</i>
							</c:if> <c:if test="${not empty post.ftags}">
								<c:forEach items="${post.ftags}" var="ftag">
									<span onclick="goToFeed(${ftag.mno})" class="tagName c-pointer">
										${ftag.nickname} </span>
								</c:forEach>
							</c:if></li>
					</ul>
				...
				<%-- dropDownBtn 들어갈 부분 --%>
				<c:if test="${targetUser.mno == loginUser.mno}">
					<i class="fas fa-ellipsis-v c-pointer dropCss"
						id="dropdown-${post.pstno}" data-toggle="dropdown"
						aria-haspopup="true" aria-expanded="false"></i>
					<!--  <a class="dropdown-toggle c-pointer" id="dropdown01" 
                  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              </a> -->
					<div class="dropdown-menu dropdown-flex"
						aria-labelledby="dropdown-${post.pstno}">
						<c:choose>
